...
void main() {
  ...
  offset += (texture2D(texture, vertTexCoord.xy).r + texture2D(texture, vertTexCoord.xy).g + texture2D(texture, vertTexCoord.xy).b) / 3;
  offset += 0.5;
  offset = offset * 75;

  vec4 newNormal = vec4(-normal.x * offset.x, -normal.y * offset.y, -normal.z * offset.z, 0.0);
  vec4 newVertex = vertex + newNormal;
